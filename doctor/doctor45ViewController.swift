...
struct  data_struct45{
    var d_name:String = ""
    var d_experience:String = ""
    var d_skill:String = ""
    var d_license:String = ""
}
class doctor45ViewController: UIViewController {

    @IBOutlet weak var tableview_doctor: UITableView!
    
    var doctor45_array:[data_struct45] = Array()
...
extension doctor45ViewController{
    func load_data45() {

        Alamofire.request("https://zh-tw.sltung.com.tw/tung/Salu/department.php?d_id=38").responseString { response in
            if let html = response.result.value{
                self.parsehtml45(html)
            }
        }
    }
    func parsehtml45(_ html1:String)  {
        var gg:NSString = " "
...
extension doctor45ViewController: UITableViewDelegate,UITableViewDataSource{
    func numberOfSections(in tableView: UITableView) -> Int {
        return 1
    }
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        if  doctor45_array.count > 0 {
            return doctor45_array.count
        }else{
               return 0
        }
