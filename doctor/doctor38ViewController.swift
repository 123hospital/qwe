// 光田 內分泌及新陳代謝科 蘇楓琳 name,experience
import UIKit
import Alamofire
import Kanna

struct  data_struct38{
    var d_name:String = ""
    var d_experience:String = ""
    var d_skill:String = ""
    var d_license:String = ""
}
class doctor38ViewController: UIViewController {

    @IBOutlet weak var tableview_doctor: UITableView!
    var doctor38_array:[data_struct38] = Array()
...
extension doctor38ViewController{
    func load_data38() {

        Alamofire.request("http://www.ktgh.com.tw/HRE_VSDPT_Look.asp?CatID=120&ModuleType=Y&NewsID=17").responseString { response in
            if let html = response.result.value{
                self.parsehtml38(html)
            }
        }
    }
    func parsehtml38(_ html1:String)  {
        var gg:NSString = " "
...
extension doctor38ViewController: UITableViewDelegate,UITableViewDataSource{
    func numberOfSections(in tableView: UITableView) -> Int {
        return 1
    }
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        if  doctor38_array.count > 0 {
            return doctor38_array.count
        }else{
               return 0
        }
