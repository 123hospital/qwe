...
struct  data_struct36{
    var d_name:String = ""
    var d_experience:String = ""
    var d_skill:String = ""
    var d_license:String = ""
}
class doctor36ViewController: UIViewController {

    @IBOutlet weak var tableview_doctor: UITableView!
    var doctor36_array:[data_struct36] = Array()
...
extension doctor36ViewController{
    func load_data36() {

        Alamofire.request("http://www.ktgh.com.tw/web/obgyn/").responseString { response in
            if let html = response.result.value{
                self.parsehtml36(html)
            }
        }
    }
    func parsehtml36(_ html1:String)  {
        var gg:NSString = " "
...
extension doctor36ViewController: UITableViewDelegate,UITableViewDataSource{
    func numberOfSections(in tableView: UITableView) -> Int {
        return 1
    }
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        if  doctor36_array.count > 0 {
            return doctor36_array.count
        }else{
               return 0
        }
