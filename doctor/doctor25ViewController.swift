...
struct  data_struct25{
    var d_name:String = ""
    var d_experience:String = ""
    var d_skill:String = ""
    var d_license:String = ""
}
class doctor25ViewController: UIViewController {

    @IBOutlet weak var tableview_doctor: UITableView!
    var doctor25_array:[data_struct25] = Array()
...
extension doctor25ViewController{
    func load_data25() {

        Alamofire.request("http://www.ktgh.com.tw/web/Cardiology/").responseString { response in
            if let html = response.result.value{
                self.parsehtml25(html)
            }
        }
    }
    func parsehtml25(_ html1:String)  {
        var gg:NSString = " "
...
extension doctor25ViewController: UITableViewDelegate,UITableViewDataSource{
    func numberOfSections(in tableView: UITableView) -> Int {
        return 1
    }
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        if  doctor25_array.count > 0 {
            return doctor25_array.count
        }else{
               return 0
        }
