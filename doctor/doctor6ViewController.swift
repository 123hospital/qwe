...
class doctor6ViewController: UIViewController {

    @IBOutlet weak var tableview_doctor: UITableView!
    var doctor6_array:[data_struct6] = Array()
...
extension doctor6ViewController{
    func load_data6() {

        Alamofire.request("http://www.ktgh.com.tw/web/Rehabilitation/").responseString { response in
            if let html = response.result.value{
                self.parsehtml6(html)
            }
        }
    }
    func parsehtml6(_ html1:String)  {
        var gg:NSString = " "
...
extension doctor6ViewController: UITableViewDelegate,UITableViewDataSource{
    func numberOfSections(in tableView: UITableView) -> Int {
        return 1
    }
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        if  doctor6_array.count > 0 {
            return doctor6_array.count
        }else{
               return 0
        }
