...
struct  data_struct44{
    var d_name:String = ""
    var d_experience:String = ""
    var d_skill:String = ""
    var d_license:String = ""
}
class doctor44ViewController: UIViewController {

    @IBOutlet weak var tableview_doctor: UITableView!
    var doctor44_array:[data_struct44] = Array()
...
extension doctor44ViewController{
    func load_data44() {

        Alamofire.request("https://zh-tw.sltung.com.tw/tung/Salu/department.php?d_id=34").responseString { response in
            if let html = response.result.value{
                self.parsehtml44(html)
            }
        }
    }
    func parsehtml44(_ html1:String)  {
        var gg:NSString = " "
...
extension doctor44ViewController: UITableViewDelegate,UITableViewDataSource{
    func numberOfSections(in tableView: UITableView) -> Int {
        return 1
    }
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        if  doctor44_array.count > 0 {
            return doctor44_array.count
        }else{
               return 0
        }
