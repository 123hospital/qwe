...
struct  data_struct22{
    var d_name:String = ""
    var d_experience:String = ""
    var d_skill:String = ""
    var d_license:String = ""
}
class doctor22ViewController: UIViewController {

    @IBOutlet weak var tableview_doctor: UITableView!
    
    var doctor22_array:[data_struct22] = Array()
...
extension doctor22ViewController{
    func load_data22() {

        Alamofire.request("https://zh-tw.sltung.com.tw/tung/Salu/department.php?d_id=5").responseString { response in
            if let html = response.result.value{
                self.parsehtml22(html)
            }
        }
    }
    func parsehtml22(_ html1:String)  {
        var gg:NSString = " "
...
extension doctor22ViewController: UITableViewDelegate,UITableViewDataSource{
    func numberOfSections(in tableView: UITableView) -> Int {
        return 1
    }
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        if  doctor22_array.count > 0 {
            return doctor22_array.count
        }else{
               return 0
        }
