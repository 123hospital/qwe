// 童綜合 兒童醫學部 小兒神經科 遲景上 教學副院長
import UIKit
import Alamofire
import Kanna

struct  data_struct18{
    var d_name:String = ""
    var d_experience:String = ""
    var d_skill:String = ""
    var d_license:String = ""
}
class doctor18ViewController: UIViewController {

    @IBOutlet weak var tableview_doctor: UITableView!
    
    var doctor18_array:[data_struct18] = Array()
...
extension doctor18ViewController{
    func load_data18() {

        Alamofire.request("https://zh-tw.sltung.com.tw/tung/Salu/department.php?d_id=23").responseString { response in
            if let html = response.result.value{
                self.parsehtml18(html)
            }
        }
    }
    func parsehtml18(_ html1:String)  {
        var gg:NSString = " "
...
extension doctor18ViewController: UITableViewDelegate,UITableViewDataSource{
    func numberOfSections(in tableView: UITableView) -> Int {
        return 1
    }
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        if  doctor18_array.count > 0 {
            return doctor18_array.count
        }else{
               return 0
        }
