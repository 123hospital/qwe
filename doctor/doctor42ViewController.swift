...
struct  data_struct42{
    var d_name:String = ""
    var d_experience:String = ""
    var d_skill:String = ""
    var d_license:String = ""
}
class doctor42ViewController: UIViewController {

    @IBOutlet weak var tableview_doctor: UITableView!
    var doctor42_array:[data_struct42] = Array()
...
extension doctor42ViewController{
    func load_data42() {

        Alamofire.request("https://zh-tw.sltung.com.tw/tung/Salu/department.php?d_id=1").responseString { response in
            if let html = response.result.value{
                self.parsehtml42(html)
            }
        }
    }
    func parsehtml42(_ html1:String)  {
        var gg:NSString = " "
...
extension doctor42ViewController: UITableViewDelegate,UITableViewDataSource{
    func numberOfSections(in tableView: UITableView) -> Int {
        return 1
    }
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        if  doctor42_array.count > 0 {
            return doctor42_array.count
        }else{
               return 0
        }
